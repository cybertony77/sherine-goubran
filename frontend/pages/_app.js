...
const brandTheme = createTheme({
  ...
  colors: {
    brand: [
      BRAND_PALETTE.surface,
      BRAND_PALETTE.softAccent,
      '#E2D3C0',
      '#D6C2A3',
      '#C9A96A',
      BRAND_PALETTE.secondary,
      BRAND_PALETTE.secondaryHover,
      BRAND_PALETTE.secondaryPressed,
      '#8F7340',
  ...
  black: BRAND_PALETTE.primary,
  white: BRAND_PALETTE.white,
  other: {
    surface: BRAND_PALETTE.surface,
    softAccent: BRAND_PALETTE.softAccent,
    bodyText: BRAND_PALETTE.bodyText,
    mutedText: BRAND_PALETTE.mutedText,
  },
});

const SYSTEM_BG_STORAGE_KEY = 'system-page-bg';

const APP_SPINNER_RING = {
  width: '50px',
  height: '50px',
  border: '4px solid rgba(201, 169, 106, 0.22)',
  borderTop: `4px solid ${BRAND_PALETTE.secondary}`,
  borderRadius: '50%',
...
function applySystemBrandColors({ primary, secondary } = {}) {
  if (typeof document === 'undefined') return;
  if (primary) {
    document.documentElement.style.setProperty('--system-primary', primary);
  }
  if (secondary) {
    document.documentElement.style.setProperty('--system-secondary', secondary);
  }
}
...
function AppContent({ Component, pageProps, systemBackground }) {
  ...
  useEffect(() => {
    ...
    const ensureEnvBackground = async () => {
      try {
        ...
        if (!cancelled) {
          applySystemBrandColors({
            primary: data?.primary_color || data?.SYSTEM_PRIMARY_COLOR,
            secondary: data?.secondary_color || data?.SYSTEM_SECONDARY_COLOR,
          });
        }
      } catch {
        /* ignore */
  ...
  const noHeaderFooterPages = useMemo(
    () => [
      '/login',
  ...
  // Generic spinner stays for dashboard / token pages only.
  // Public portfolio routes use PageTransitionProvider instead.
  useEffect(() => {
  ...
  // Dashboard is available to all authenticated staff roles (and any logged-in user)
  // Student-only redirect away from /dashboard was removed for this website.
  ...
  // Show loading while checking authentication, subscription, or during protected route changes.
  // Public portfolio pages keep rendering underneath the branded page transition.
  const isCurrentPublicSite = isPublicSitePage(router.pathname);
  if (
    (isLoading && !isCurrentPublicSite) ||
...
App.getInitialProps = async (appContext) => {
  ...
  if (typeof window === 'undefined') {
    try {
      // Server-only: keep fs out of the client bundle
      // eslint-disable-next-line global-require
      const { loadSystemBackgroundFromEnv } = require('../lib/systemColors.server');
