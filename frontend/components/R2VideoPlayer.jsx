...
export default function R2VideoPlayer({
  ...
  const videoOrientation = useMemo(() => orientationFromAspectRatio(aspectRatio), [aspectRatio]);
  ...
  }, [r2Key, videoApiKey, fillParent]);

  useEffect(() => {
    if (fillParent) return undefined;
    onAspectRatioChange?.(aspectRatio, videoOrientation);
  }, [aspectRatio, videoOrientation, onAspectRatioChange, fillParent]);

  useEffect(() => {
    if (fillParent) return undefined;
    const video = videoRef.current;
    if (!video || !presignedUrl) return undefined;

    const syncAspectRatio = () => {
      setAspectRatio(aspectRatioFromVideo(video));
    };
  ...
  if (isLoadingUrl && !presignedUrl) {
    return (
      <div
        className={fillParent ? "video-player-root video-player-fill" : "video-player-root"}
        data-video-orientation={videoOrientation}
        data-player-layout={fillParent ? "fill" : "intrinsic"}
        style={{
          ...shellStyle,
          borderRadius: fillParent ? 0 : "10px",
        }}
      >
  ...
  if (error) {
    return (
      <div
        ...
        justifyContent: 'center',
        color: '#dc3545',
        fontSize: fillParent ? '0.72rem' : '1rem',
  ...
  return (
    <div
      ...
      data-video-orientation={videoOrientation}
      data-player-layout={fillParent ? "fill" : "intrinsic"}
      className={fillParent ? "video-player-root video-player-fill" : seekContainerProps.className}
      style={{
        ...shellStyle,
        outline: "none",
      }}
